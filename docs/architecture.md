...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij and cmux remain unverified, and Orca does not support secondmate spawns.
...
cmux's container shape is one workspace per task with one surface, no per-home container split; workspace titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path, and a `--secondmate` spawn creates one dedicated workspace whose cwd is the secondmate's home ([`cmux-backend.md`](cmux-backend.md#secondmate-launch)).
